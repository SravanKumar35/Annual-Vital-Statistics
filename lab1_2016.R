...
a=read.csv(file = "G:\\Required\\6th Sem\\DA\\Lab\\Lab1\\2015.csv",head=TRUE, sep = ",", skip=4,nrows = 44)
a = Filter(function(x)!all(is.na(x)), a)
a
summary(a)
var(a)
sd(a$Live.births)
...
ggplot(a,aes(x=a$Year,y=a$Deaths))+  geom_line() + xlim(1970, 2014) + scale_y_continuous(limits=c(87000,420000))
ggplot(a,aes(x=a$CRS.Births,y=a$CRS.Deaths))+  geom_line()
ggplot(a, aes(x=a$Year, y=a$Live.births)) + geom_line()
ggplot(a, aes(x=a$Year, y=a$Still.births)) + geom_col()
ggplot(a, aes(x=a$Year,y=a$Deaths, group=1)) + geom_boxplot()
IQR(a$Deaths)

outlier=boxplot.stats(a$Still.births)$out
boxplot(a$Still.births, main="Deaths", boxwex=0.1)
...
b=read.csv(file = "G:\\Required\\6th Sem\\DA\\Lab\\Lab1\\2014.csv",head=TRUE, sep = ",", skip=53, nrows=30)
b
summary(b)
var(b$Birth_Registered)
var(b$Birth_Rate)
var(b$Death_Registered)
var(b$Death_Rate)
var(b$Registered.Infant.Death)
var(b$Still.Birth_Registered)
var(b$Still.Birth_Rate)

sd(b$Birth_Registered)
sd(b$Birth_Rate)
sd(b$Death_Registered)
sd(b$Death_Rate)
sd(b$Registered.Infant.Death)
sd(b$Still.Birth_Registered)
sd(b$Still.Birth_Rate)

plot(b)

ggplot(b, aes(x=b$Birth_Registered,y=b$Death_Registered)) + geom_line()
ggplot(b, aes(x=b$Birth_Rate, y=b$Death_Rate)) + geom_line()
ggplot(b, aes(x=b$Still.Birth_Rate, y=b$Still.Birth_Registered)) + geom_col()

pie(b$Registered.Infant.Death)

barplot(b$Registered.Infant.Death, names.arg = b$District, col = b$District, legend=b$District)
barplot(b$Birth_Registered, names.arg = b$District)
barplot(b$Death_Registered, names.arg = b$District)
barplot(b$Registered.Infant.Death, names.arg = b$District)


outlier1=boxplot.stats(b$Birth_Registered)$out
boxplot(b$Birth_Registered, main="Births Registered", boxwex=0.1)
...
outlier3=boxplot.stats(b$Death_Rate)$out
boxplot(b$Death_Rate, main="Death Rate", boxwex=0.1)
mtext(paste("Outliers: ", paste(outlier3, collapse = ", ")),cex=0.6)

outlier4=boxplot.stats(b$Registered.Infant.Death)$out
boxplot(b$Registered.Infant.Death, main="Registered Infant Death", boxwex=0.1)
...
c=read.csv(file = "G:\\Required\\6th Sem\\DA\\Lab\\Lab1\\2014.csv",head=TRUE, sep = ",", skip=89, nrows=30)
c
summary(c)
...
d=read.csv(file = "G:\\Required\\6th Sem\\DA\\Lab\\Lab1\\2014.csv",head=TRUE, sep = ",", skip=125, nrows=30)
d
summary(d)
...
var(d$Still.Birth_Registered)
var(d$Still.Birth_Rate)
...
sd(d$Still.Birth_Registered)
sd(d$Still.Birth_Rate)
...
ggplot(d, aes(x=d$Still.Birth_Rate, y=d$Still.Birth_Registered)) + geom_col()
...
e=read.csv(file = "G:\\Required\\6th Sem\\DA\\Lab\\Lab1\\2014.csv",head=TRUE, sep = ",", skip=165, nrows=30)
e
summary(e)
...
var(e$No.of.Monthly.Returns.due)
...
sd(e$No.of.Monthly.Returns.due)
...
plot(e)
hist(e$Actual)
ggplot(e, aes(x=e$No.of.Registration.Units,y=e$No.of.Monthly.Returns.due)) + geom_line()
...
boxplot(e$No.of.Monthly.Returns.due, main="No of monthly Returnd Due", boxwex=0.1)
...
f=read.csv(file = "G:\\Required\\6th Sem\\DA\\Lab\\Lab1\\2014.csv",head=TRUE, sep = ",", skip=207, nrows=30)
f
summary(f)
